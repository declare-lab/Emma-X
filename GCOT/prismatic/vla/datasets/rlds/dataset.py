...
def _make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        meta_data = traj.pop("traj_metadata").pop("episode_metadata")
        filepath = meta_data.pop("file_path")
        episode_id = meta_data.pop("episode_id")
...
def make_interleaved_dataset(
    ...
    *,
    ...
    dataset_len = int(
        (np.array(dataset_sizes) / sample_weights)[primary_dataset_indices].max()
    )
...
def load_bridgev2_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        if "dataset_frame_transform_kwargs" in data_kwargs:
            data_kwargs.pop("dataset_frame_transform_kwargs")
        _, dataset_statistics = _make_dataset_from_rlds(**data_kwargs, train=train)
        dataset_sizes.append(dataset_statistics["num_trajectories"])
        all_dataset_statistics[dataset_kwargs["name"]] = dataset_statistics
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset, dataset_statistics = _make_dataset_from_rlds(
            **dataset_kwargs,
            train=train,
            num_parallel_calls=threads,
            num_parallel_reads=reads,
