        ...
        while t < cfg.max_steps:
            try:
                ...
                if curr_tstamp > last_tstamp + step_duration:
                    ...
                    delta, reasoning = get_action(cfg, model, obs, task_input, processor=processor, type="pos")
                    delta_position, delta_gripper = delta[:6], delta[-1]
                    # very important step here
                    delta_position = np.array(delta_position)
                    current_position = mat_to_pose(obs["eef_transform"])
                    # for rotation and gripper, we use the absolute value
                    current_position[3:] = 0
                    goal_position = delta_position + current_position
                    goal_mat = pose_to_mat(goal_position)
                    goal_gripper = delta_gripper
                    print("delta", delta)
                    ...
                    obs, _, _, _, _ = env.move(goal_mat, goal_gripper, duration=3)
