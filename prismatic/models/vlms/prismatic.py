...
class PrismaticVLM(VLM):
    ...
    def generate_actions(self, image: Image, prompt_text: str, type: str, **kwargs: str) -> str:
        ...
        if type == "act":
            ...
            require_unorm, delta_position = s.extract_movement_plan(generated_text)
            if require_unorm:
                proprio_norm_stats = self.get_proprio_stats()
                mask = proprio_norm_stats.get("mask", np.ones_like(proprio_norm_stats["Q1"], dtype=bool))
                proprio_high, proprio_low = np.array(proprio_norm_stats["Q99"]), np.array(proprio_norm_stats["Q1"])
                proprio_norm = np.where(
                    mask, 0.5 * (np.array(delta_position) + 1) * (proprio_high - proprio_low) + proprio_low, delta_position
                )
            return proprio_norm, generated_text
