...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            ...
            "relative_l1_loss": deque(maxlen=window_size),
    ...
    def get_status(self, loss: Optional[torch.Tensor] = None, acc = None) -> str:
        lr = self.state["lr"][-1] if len(self.state["lr"]) > 0 else 0
        if loss is None:
            ...
            if acc is None:
                ...
                return f"=>> [Epoch {self.epoch:03d}] Global Step {self.global_step:06d} =>> LR :: {lr:.6f} - Loss :: {loss:.4f} - Acc :: {acc:.4f}"
    ...
    def push(self) -> str:
        ...
        step_time, lr = np.mean(list(self.state["step_time"])), self.state["lr"][-1]
        status = self.get_status(loss, acc=action_accuracy)
...
class VLADiscreteMetrics:
    def __init__(
        ...
        self.state = {
            ...
            "relative_l1_loss": deque(maxlen=window_size),
            "action_accuracy": deque(maxlen=window_size),
            "state_accuracy": deque(maxlen=window_size),
            "l1_loss_val": deque(maxlen=window_size),
            "relative_l1_loss_val": deque(maxlen=window_size),
            "action_accuracy_val": deque(maxlen=window_size),
            "state_accuracy_val": deque(maxlen=window_size),
            "step_time": deque(maxlen=window_size),
            "lr": [],
        }

        # Created metrics buffers for individual tracked datasets
        # self.eval_trackers = defaultdict(lambda: VLAMetrics([], "", "", {}))
        ...
        **kwargs,
        ...
        for key, value in kwargs.items():
            if key == "loss":
                ...
                self.state["loss_raw"].append(loss_val)
                self.state["loss"].append(loss_val)
            # elif key in ["state_accuracy", "action_accuracy", "l1_loss",]:
    ...
    def push(self) -> str:
        ...
        loss = torch.stack(list(self.state["loss"])).mean().item()
        l1_loss = torch.stack(list(self.state["l1_loss"])).mean().item()
        relative_l1_loss = torch.stack(list(self.state["relative_l1_loss"])).mean().item()
        action_accuracy = torch.stack(list(self.state["action_accuracy"])).mean().item()
        state_accuracy = torch.stack(list(self.state["state_accuracy"])).mean().item()
        if len(self.state["l1_loss_val"]) == 0:
            l1_loss_val = 0
            relative_l1_loss_val = 0
            action_accuracy_val = 0
            state_accuracy_val = 0
        else:
            l1_loss_val = torch.stack(list(self.state["l1_loss_val"])).mean().item()
            relative_l1_loss_val = torch.stack(list(self.state["relative_l1_loss_val"])).mean().item()
            action_accuracy_val = torch.stack(list(self.state["action_accuracy_val"])).mean().item()
            state_accuracy_val = torch.stack(list(self.state["state_accuracy_val"])).mean().item()
        step_time, lr = np.mean(list(self.state["step_time"])), self.state["lr"][-1]
        status = self.get_status(loss)
        ...
        #     dataset_metrics.update(
        #         {
        #             f"VLA Eval/L1 Loss": torch.stack(list(tracker.state["l1_loss"])).mean().item(),
        #             f"VLA Eval/Action Token Accuracy": torch.stack(list(tracker.state["action_accuracy"])).mean().item(),
        #         }
        #     )
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Loss": loss,
                f"{prefix}/L1 Loss": l1_loss,
                f"{prefix}/relative L1 Loss": relative_l1_loss,
                f"{prefix}/Action Token Accuracy": action_accuracy,
                f"{prefix}/Next State Accuracy": state_accuracy,
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
                "VLA Val/L1 Loss": l1_loss_val,
                "VLA Val/relative L1 Loss": relative_l1_loss_val,
                "VLA Val/Action Token Accuracy": action_accuracy_val,
                "VLA Val/Next State Accuracy": state_accuracy_val,
                # **dataset_metrics,
            },
        )
        return status
