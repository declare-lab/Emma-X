...
class FastDatasetDiscrete(Dataset):
    def __init__(
        ...
        dataset, dataset_len, self.dataset_statistics = bridge_v2_dataset(split="val")
        with open(f"{data_root_dir}/{file_name}", "r") as file:
            self.data = json.load(file)

        import time
        time.sleep(60)
        self.traj_list = np.load("/dev/shm/traj_array.npy", mmap_mode='r')
        self.length = np.load("/dev/shm/lengths.npy", mmap_mode='r')
        with open('/dev/shm/traj_keys.json') as f:
            self.traj_keys = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        traj_index_start, traj_index_end = self.traj_keys[key]
        byte_len = self.length[traj_index_start: traj_index_end][current_image_index]
        traj_image = self.traj_list[traj_index_start: traj_index_end][current_image_index][:byte_len].tobytes()
        image = tf.io.decode_image(
            traj_image, expand_animations=False, dtype=tf.uint8
        ).numpy()
...
if __name__ == "__main__":
    ...
    def build_dataset(dataset):
        for i, traj in tqdm(enumerate(dataset.as_numpy_iterator())):
            key = traj["file_path"][0].decode("utf-8") + "|" + str(traj["episode_id"][0])
            traj_images[key] = traj["observation"]["image_primary"]

    dataset, dataset_len, dataset_statistics = bridge_v2_dataset(split="train")
    build_dataset(dataset)
    dataset, dataset_len, dataset_statistics = bridge_v2_dataset(split="val")
    build_dataset(dataset)

    print(len(traj_images))

    # Generate trajectory images and keys
    traj_list = []
    traj_keys = dict()
    start = 0

    for key in traj_images.keys():
        traj_list.append(traj_images[key])
        length = len(traj_images[key])
        traj_keys[key] = (start, start + length)
        start += length

    # Save trajectory keys
    with open('/dev/shm/traj_keys.json', 'w') as f:
        json.dump(traj_keys, f)

    # Process the trajectory list directly
    traj_list = np.concatenate(traj_list).reshape(-1)

    # Create data array and lengths
    max_len = 45000
    data_array = np.zeros((len(traj_list), max_len), dtype=np.uint8)
    lengths = np.zeros(len(traj_list), dtype=np.int32)

    for i in range(len(traj_list)):
        byte_data = traj_list[i]
        l = len(byte_data)
        lengths[i] = l
        data_array[i, :l] = np.frombuffer(byte_data, dtype=np.uint8)

    # Save data array and lengths
    np.save("/dev/shm/traj_array.npy", data_array)
    np.save("/dev/shm/lengths.npy", lengths)
