...
from prismatic.training import VLADiscreteMetrics, get_train_strategy
from prismatic.util import set_global_seed
from prismatic.vla import get_discrete_dataset_and_collator
...
class TrainConfig:
    ...
    vla: VLAConfig = field(
        default_factory=VLAConfig.get_choice_class(VLARegistry.PHI_BRIDGE.vla_id)
    )
    ...
    wandb_entity: str = "emryshong"                          # Name of entity to log under
...
def train(cfg: TrainConfig) -> None:
    ...
    with open(f"{cfg.data_root_dir}/dataset_statistics.json") as f:
    ...
    if cfg.pretrained_checkpoint is not None:
        ...
        vlm = load_vla(cfg.pretrained_checkpoint, hf_token=hf_token, load_for_training=True, proprio_norm_stats=proprio_norm_stats)

    else:
        raise NotImplementedError("Checkpoint Resuming is not yet supported in text-how-to-go!")
    ...
    vla_dataset, vla_val_dataset, collator, val_collator, action_tokenizer = get_discrete_dataset_and_collator(
        cfg.data_root_dir,
        cfg.vla.data_mix,
    ...
    if overwatch.is_rank_zero():
        print(vla_dataset.dataset_statistics)
        save_dataset_statistics(vla_dataset.dataset_statistics, run_dir)

    # Create Train Strategy
    overwatch.info(f"Initializing Train Strategy `{cfg.train_strategy}`")
    # print(cfg.vla.enable_gradient_checkpointing)
    # input()
    ...
    train_strategy.run_discrete_vla_training(
        vla_dataset=vla_dataset,
        val_dataset = vla_val_dataset,
        collator=collator,
